...
levels::levels(RenderWindow *window, Font *font)
{
    for(int i=0;i<3;i++)
    {
        for(int j=0;j<5;j++)
        {
            ...
            level.push_back( button( 100+j*132.5,150+i*132.5,70,70,40,font,ch,Color(255,160,i*25+j*20+122)));
        }
    }
}
...
void levels::level_won(int n)
{
    auto a = level.begin();
    if(n>1)
        for (int i=1; i<n ; i++)
            a++;
    a->shape.setFillColor(Color(255,228,181));
}
